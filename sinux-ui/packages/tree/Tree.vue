<template>
<div class="el-tree" :class="{
      'el-tree--highlight-current': highlightCurrent
    }" role="Tree" @keydown="handleKeydown">
  <template v-if="showRootNode">
    <el-tree-node :node="tree.root" :key="tree.root.id"> </el-tree-node>
  </template>
  <template v-else>
    <el-tree-node v-for="child in tree.root.childNodes" :node="child" :key="child.id">
    </el-tree-node>
  </template>
  ...
  <div v-show="
      ...
      " class="el-tree__drop-indicator" ref="dropIndicator"></div>
</div>
</template>
...
import {
  t
} from 'sinux-ui/src/locale'
import {
  isFunction
} from 'sinux-ui/src/utils/types'
import {
  extractMethods
} from './libs/util'
